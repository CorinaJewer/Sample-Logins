const dal = require("./pg.auth_db");  // Database Connection

//get all logins.
async function getLogins () {
  if(DEBUG) console.log("pg.logins.dal.getLogins()");
  const sql = `SELECT id AS _id, username, password, email, uuid FROM public."Logins" ORDER BY id DESC LIMIT 7;`
  try{
...
async function patchLogin(id, username, password,email) {
  ...
  try { 
    let result = await dal.query(sql, [id, username, password, email]);
    // if(DEBUG) console.log(result);
